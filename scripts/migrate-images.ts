...
// URLをローカルパスに変換
function convertUrlToLocalPath(url: string): string {
...
async function processAttachments() {
  ...
  const attachments: Attachment[] = JSON.parse(fs.readFileSync(ATTACHMENTS_FILE, 'utf-8'));
      ...
      for (const [sizeKey, sizeData] of Object.entries(attachment.sizes)) {
        if (sizeData && sizeData.url && sizeData.url.startsWith(BASE_URL)) {
          ...
          try {
            ...
            (updatedSizes as any)[sizeKey] = {
              ...sizeData,
              url: localPath
            };
          } catch (error) {
            console.error(`Failed to download ${sizeData.url}:`, error);
            // ダウンロードに失敗した場合は元のURLを保持
          }
        }
      }
